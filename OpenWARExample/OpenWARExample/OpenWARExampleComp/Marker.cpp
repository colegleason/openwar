...
void Marker::UpdateForRenderResolutionChange(float width, float height)
{
	...
	{
		(*it)->UpdateForRenderResolutionChange(width, height);
	}
}

void Marker::UpdateForWindowSizeChange(float width, float height)
{
	...
	for (vector<Model^>::iterator it = m_ModelList.begin(); it != m_ModelList.end(); ++it)
	{
		(*it)->UpdateForWindowSizeChange(width, height);
	}
}
