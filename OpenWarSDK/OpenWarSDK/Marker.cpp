...
void Marker::AddModel(Model^ nmodel)
{
	nmodel->Initialize(m_window.Get());
	nmodel->SetDeviceAndContext((int)m_d3dDevice.Get(), (int)m_d3dContext.Get());
	m_ModelList.push_back(nmodel);
}

void Marker::Initialize(Windows::UI::Core::CoreWindow^ window)
{
	m_window = window;
...
void Marker::CreateWindowSizeDependentResources()
{
...
void Marker::UpdateForWindowSizeChange()
{
...
void Marker::ReleaseResourcesForSuspending()
{
...
float Marker::ConvertDipsToPixels(float dips)
{
